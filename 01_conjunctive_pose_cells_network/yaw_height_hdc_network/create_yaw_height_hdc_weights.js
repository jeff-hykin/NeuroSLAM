export function createYawHeightHdcWeights(yawDim, heightDim, yawVar, heightVar) {
    // Creates a 2D normalised distributio of size dim^2 
    ...
    for (let h = 0; h < heightDim; h++) {
        for (let y = 0; y < yawDim; y++) {
            ...
            weight[y][h] = (
                (
                    1 / (yawVar * Math.sqrt(2 * Math.PI))
                ) * (
                    Math.exp(
                        -Math.pow(y - yawDimCentre, 2)
                        /
                        (2 * Math.pow(yawVar, 2))
                    )
                ) * (
                    1 / (heightVar * Math.sqrt(2 * Math.PI))
                ) * (
                    Math.exp(
                        -Math.pow(h - heightDimCentre, 2)
                        /
                        (2 * Math.pow(heightVar, 2))
                    )
                )
            )
        }
